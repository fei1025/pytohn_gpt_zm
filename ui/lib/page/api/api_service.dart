...
class ApiService{
  ...
  static Future<List<ChatHist>> fetchData() async {
    final response = await http.get(Uri.parse('$_baseUrl/getAllHist'),
    ...
    if (response.statusCode == 200) {
      ...
      for (ChatHist item in apiData.data) {
        print('Model: ${item.model}, Chat ID: ${item.chatId}, Title: ${item.title}, Creation Time: ${item.creationTime}');
      }
      return apiData.data;
    } else {
